...
当前我们对于给定的7组数据有如下结果，搜索深度未注明均为8层，未进行剪枝：
...
部分图的结果不够完善，即使增大搜索层数到12层的地步都没有能够很好地得到结果，问题可能出在剪枝上，没有能够对某些明显不合理的地方做出舍弃从而留出时间给某些可能性更大的结果。具体而言，对于钥匙的处理没有预期处理的好，这一点需要继续改进。

另一个不足之处是`eval`函数，没有能够调整的非常精细，对于某些权值的理解可能还不够深入，所以导致可能会在某些关键位置决策错误，从而影响后面的继续搜索。
